...
Use the [Get Rate](/api-reference/rate/get-rate) endpoint to fetch current conversion rates.
...
You can also create a custom rate key using the [Create Ratekey](/api-reference/rate/create-ratekey) endpoint if you need to apply a custom spread or margin. Verify a rate key is still valid using [Verify Ratekey](/api-reference/rate/verify-ratekey).
...
Configure the default fee bearer in your merchant settings via the [dashboard](https://dashboard.getpartna.com) or the [Update Settings](/api-reference/user/update-settings) endpoint.