...
**"Invalid rate key"**: The rate key has expired or is malformed. Fetch a fresh rate using [Get Rate](/api-reference/rate/get-rate) and use the new key.

### KYC errors (422)

**"KYC not completed"**: The user account has not passed identity verification. Complete the KYC flow before initiating transactions. See [Accounts and KYC](/guides/accounts-and-kyc).
