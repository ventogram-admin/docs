...
<Steps>
  <Step title="Configure your merchant settings">
    ...
    Configure these via the [dashboard](https://dashboard.getpartna.com) or the [Update Settings](/api-reference/user/update-settings) endpoint.
  </Step>
  <Step title="Create a user account for your customer">
    Register each customer using [Create Account](/api-reference/account/create-account). This creates a permanent payment account that your customer can deposit to at any time.
  </Step>
  ...
  <Step title="Initiate a collection">
    Use the onramp flow to collect a payment. Call [Onramp and Offramp](/api-reference/ramp/onramp-and-offramp) with `type: "fiatToCrypto"` to create a payment request. The response includes the bank account details where your customer should send their payment.

    Alternatively, if you are using the collect API for simpler collection without crypto conversion, you can create a payment using [Create Transfer](/api-reference/transfer/create-transfer) or [Initiate Transfer](/api-reference/transfer/initiate-transfer).
  </Step>
...
Use [Get Balances](/api-reference/user/get-balances) to check your merchant account balances across all currencies:
...
Once funds are settled in your account, use [Withdraw](/api-reference/transfer/withdraw) to send funds to an external wallet or bank account.

## Currency conversion

If you hold balances in one currency and need to convert to another, use [Convert Currency](/api-reference/transfer/convert-currency) to convert between supported currencies within your account.
