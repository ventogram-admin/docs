...
| `voucher.update.declined`     | Voucher payment failed due to the reason which will be sent in the webhook as `paymentFailureReason`
| `voucher.cancelled`     | Available in V2. This event will be raised after a user has cancelled an unpaid voucher
...
````
```javascript voucher.update.declined
{
  "event": "voucher.update.declined";
  "data": {
    "id": string;
    "merchant": string;
    "amount": number;
